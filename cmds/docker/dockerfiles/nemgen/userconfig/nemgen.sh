...
if [ ! -d /data/00000 ]; then
  ...
  # first time to get jz.jzj and jz.gas
  ...
  jz_mosaic_id=$(grep "jz.jzj" /tmp/nemgen.log | grep nonce  | awk -F=  '{split($0, a, / /); print a[9]}' | sort -u)
  currency_mosaic_id=$(grep "jz.gas" /tmp//nemgen.log | grep nonce  | awk -F=  '{split($0, a, / /); print a[9]}' | sort -u)
